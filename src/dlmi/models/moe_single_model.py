...
class MOESingleModel(pl.LightningModule):
    ...
    def update_lr(self):
        scheduler_cnn = self.lr_schedulers()
    ...
    def infer(self, cnn_features, mlp_features, batch_size, augment=False):
        ...
        pred_final = self.final_classifier(torch.cat([pred_cnn, pred_mlp], dim=1).float())
    ...
    def configure_optimizers(self):
        ...
        optimizer_final = hydra.utils.instantiate(
            self.cfg.optimizer,
            *[chain(self.cnn.parameters(), self.mlp.parameters(), self.final_classifier.parameters())],
        ...
        return optimizer_final, scheduler_final
